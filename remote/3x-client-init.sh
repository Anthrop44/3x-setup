...
trap 'exit_code=$?; rm -f "$COOKIE_JAR" "$LOGIN_PAGE_PATH" "$LOGIN_RESPONSE_PATH" "$INBOUNDS_RESPONSE_PATH" "$CLIENTS_RESPONSE_PATH" "$DELETE_PAYLOAD_PATH" "$DELETE_RESPONSE_PATH" "$CLIENT_PAYLOAD_PATH" "$CLIENT_RESPONSE_PATH" "$RESTART_RESPONSE_PATH" "$STATUS_RESPONSE_PATH"; script_timer_finish "$exit_code"' EXIT
