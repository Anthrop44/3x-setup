...
trap 'exit_code=$?; rm -f "$COOKIE_JAR" "$LOGIN_PAGE_PATH" "$LOGIN_RESPONSE_PATH" "$INBOUNDS_RESPONSE_PATH" "$HOSTS_RESPONSE_PATH" "$HOST_PAYLOAD_PATH" "$HOST_RESPONSE_PATH" "$DELETE_PAYLOAD_PATH"; script_timer_finish "$exit_code"' EXIT
