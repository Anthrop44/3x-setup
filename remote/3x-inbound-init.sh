...
trap 'exit_code=$?; rm -f "$COOKIE_JAR" "$LOGIN_PAGE_PATH" "$LOGIN_RESPONSE_PATH" "$CERT_RESPONSE_PATH" "$HY2_PAYLOAD_PATH" "$HY2_RESPONSE_PATH" "$REALITY_PAYLOAD_PATH" "$REALITY_RESPONSE_PATH" "$XHTTP_PAYLOAD_PATH" "$XHTTP_RESPONSE_PATH" "$RESTART_RESPONSE_PATH"; script_timer_finish "$exit_code"' EXIT
