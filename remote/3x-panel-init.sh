...
trap 'exit_code=$?; rm -f "$COOKIE_JAR" "$LOGIN_PAGE_PATH" "$LOGIN_RESPONSE_PATH" "$SETTINGS_RESPONSE_PATH" "$SETTINGS_UPDATE_PATH" "$SETTINGS_UPDATE_RESPONSE_PATH" "$XRAY_RESPONSE_PATH" "$XRAY_OBJECT_PATH" "$XRAY_UPDATE_PATH" "$XRAY_UPDATE_RESPONSE_PATH" "$XRAY_RESTART_RESPONSE_PATH" "$RESTART_PANEL_RESPONSE_PATH"; script_timer_finish "$exit_code"' EXIT
