...
run_update() {
	...
	trap 'exit_code=$?; cleanup_update; script_timer_finish "$exit_code"' EXIT
...
install_task() {
	...
	trap 'exit_code=$?; rm -f "$runtime_config_temp"; script_timer_finish "$exit_code"' EXIT
	...
	sudo install -m 644 -o root -g root "$SCRIPT_DIR/script-timing.sh" "$(dirname "$INSTALL_PATH")/script-timing.sh"
