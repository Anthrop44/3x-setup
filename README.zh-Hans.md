...
- 设置`cdnDomain`高位回源端口，记为`cdnPort`
	- 进入`规则` -> `概述` -> `Origin Rules` -> `创建规则`
	- `如果传入请求匹配` -> `自定义筛选表达示` -> 编辑表达式：`http.host in {"cdnDomain"}`（把`cdnDomain`改成你真正的域名）
	- `目标端口`：重写到`cdnPort`
- SSL/TLS加密模式选择`完全（严格）`
- 启用始终使用HTTPS
- 启用TLS1.3
...
后续若其它脚本显示potential MITM或Exit code: 255等提示，可执行一次`pwsh get-log.ps1`。若能成功获取log，则可以认为安全，其它脚本也应会自动解锁
...
请定期执行此操作或重新部署代理服务器以跟进上游安全更新

## Warning

本方案可能无法在Windows 10的客户端上使用，因为Windows 10没有完整的TLS 1.3支持

本方案测试时3X-UI版本是v3.5.0。未来若3X-UI API发生变更，可能会出问题，但最好不要为了使用本方案而固定3X-UI版本。万一遇到问题请发issue

本项目不是幂等部署器，需要对重建后的干净服务器执行，不能用于已有业务的服务器

不要泄露`remote/config.json`等私有配置。**一旦泄露，请立刻重建服务器并更换ip和域名**
