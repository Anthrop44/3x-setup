...
.SYNOPSIS
	生成部署所需的SSH公钥
.DESCRIPTION
	如果 remote/id_ed25519.pub 已存在且对应 $env:USERPROFILE/.ssh/id_ed25519，则打印成功信息后跳过；如果不对应则报错，避免覆盖已有部署公钥。如果 $env:USERPROFILE/.ssh/id_ed25519 不存在，则生成一对新的Ed25519密钥；如果该私钥已存在，则从它导出公钥。最终公钥写入 remote/id_ed25519.pub，且公钥末尾comment固定为 https://github.com/Anthrop44/3x-setup
#>
